...
Each agent panel and the report section may show one of these AI result badges:

- **AI Success** - All AI steps completed successfully and the report was generated by AI.
- **Fallback** - AI could not complete one or more steps, so a simplified or partial result was used.
- **Error** - The AI step failed entirely (for example, due to a temporary service or API issue).

If you see **Fallback** or **Error**, you can retry the run or provide your own Gemini API key for higher reliability.
